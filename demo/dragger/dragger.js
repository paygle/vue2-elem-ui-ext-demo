...
var oldXY = {x:0, y:0 };  // 历史记录

var styleSet = function(el, obj, isInit){
    if (typeof el !== 'object' || typeof obj !== 'object') return;
    for(var prop in obj) {
        if(prop in el.style) {
            if(prop !== 'position') {
...
var eventMouseDown = function (e) {
    ...
    // 初始化元素位置
    if(this.el){
        styleSet(this.el, {}, true);
        oldXY = { x: e.clientX, y: e.clientY };
    }
    startDrag.call(this, oldXY);
};

var eventMouseMove = function (e) {
    if (!this.isDragging) return;
    this.el.style.left = (this.el.offsetLeft + e.clientX - oldXY.x) + 'px';
    this.el.style.top = (this.el.offsetTop + e.clientY - oldXY.y) + 'px';
    oldXY = { x: e.clientX, y: e.clientY };
    moveHandle.call(this, oldXY);
};
