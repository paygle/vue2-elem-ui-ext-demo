...
define([
  'vue',
  'text!demo/color-form/table-exam/main.tpl',
  'funcs',
  'mixins/emitter'
], function(Vue, tpl, funcs, emitter) {
  'use strict';

return Vue.component('TableExam', {
  template: tpl,
  mixins: [emitter],
  data: function() {
     // 数值验证
  ...
  methods: {
    validTrigger: function(model) {
      this.$refs.relTable.modifiedCompare();
    },
    validTriggerx: function(model) {
      this.$refs.relTablex.modifiedCompare();
    },
    // 修改后样式设置
    modifiedStyl: function(modified, row, prop, rowindex, delnum) {
      console.log('modifiedStyl:', modified, row, prop, rowindex, delnum);
      if (modified) return {todo: 'set', row: {background: 'green' },  col: {background: 'red' }};
      return {todo: 'clear', row: {background: '' },  col: {background: '' }};
    },

    tableRowClick: function(row, event, column, table) {
      table.store.commit('toggleOneRowExpanded', row);
      // this.$refs.relTable.setRowStyle('0-1', {background:'black', color: '#fff'});
    },

    tableRowClickx: function(row, event, column, table) {
      table.store.commit('toggleOneRowExpanded', row);
      // this.$refs.relTablex.setRowStyle('0-1', {background:'black', color: '#fff'});
    },
    DeleteRow: function(index, row){
      this.tableData.splice(index, 1);
    },
    DeleteRowx: function(index, row){
      this.tableDatax.splice(index, 1);
    },
    // list-complete 加载数据
    loadAll: function() {
      return [
  ...
  mounted: function(){
    ...
    this.$nextTick(function(){
      this.$refs.relTable.lockData();
      this.$refs.relTablex.lockData()
    });
  }
});
